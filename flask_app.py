...
cur = con.cursor()
cur.execute("CREATE TABLE IF NOT EXISTS mittaukset (id INTEGER PRIMARY KEY,"
            " paiva INTEGER, mittaus INTEGER)")
con.commit()
con.close()
...
def index():
    return render_template('mittaukset.html', taulukko=lampotilat,
        paivat=paivat)
...
def lisaa_tietokantaan():
    ...
    con = sqlite3.connect("mittaukset.db3")
    cur = con.cursor()
    cur.execute("INSERT INTO mittaukset (paiva, mittaus) VALUES (?,?)",
                [uusimittaus["x"], uusimittaus["y"]])
    con.commit()
    con.close()
